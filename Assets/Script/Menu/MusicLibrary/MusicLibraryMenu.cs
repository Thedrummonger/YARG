                ...
                if (SettingsManager.Settings.LibrarySort < SortAttribute.Playable)
                {
                    ...
                    if (ShouldDisplayGoalSong)
                    {
                        if (String.IsNullOrWhiteSpace(APEvents.APGoalSong.Instrument))
                            list.Add(new CategoryViewType("AP Goal Song".ToRainbowString(), 1, new SongEntry[] { goalSong }, RefreshAndReselect));
                        else
                        {
                            ...
                            list.Add(new CategoryViewType($"{"AP Goal Song".ToRainbowString()}: {instrumentDisplay}", 1, new SongEntry[] { goalSong }, RefreshAndReselect));
                        }
                    ...
                    if (AvailableSongs.Any())
                    {
                        ...
                        {
                            list.Add(new CategoryViewType("AP Songs".ToRainbowString(), availableAPSongs.Count, availableAPSongs.ToArray(), RefreshAndReselect));
                        ...
                        if (availableAPSongsWithHeader.Count > 0)
                        {
                            foreach (var header in availableAPSongsWithHeader)
                            {
                                ...
                                string instrumentDisplay = headerAquireStatus[header.Key]
                                    ? $"<color=#00FF88>{instname}</color>"
                                    : $"<color=#FF4040>{instname}</color>";
                                list.Add(new CategoryViewType($"{"AP Songs".ToRainbowString()}: {instrumentDisplay}", header.Value.Count, header.Value.ToArray(), RefreshAndReselect));
